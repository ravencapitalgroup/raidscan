...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`\n=== Processing PoiDataBinance ===`);

    const allPoiData = await base44.asServiceRole.entities.PoiDataBinance.list();
    ...
    if (recordsToDelete.length > 0) {
      for (let i = 0; i < recordsToDelete.length; i += 50) {
        ...
        for (const id of batch) {
          try {
            await base44.asServiceRole.entities.PoiDataBinance.delete(id);
            recordsDeleted++;
          } catch (err) {
    ...
    console.log(`Deleted ${recordsDeleted} old records from PoiDataBinance`);
    ...
    const existingQuarterly = await base44.asServiceRole.entities.PoiDataBinance.filter({ timeframe: '1q' });
    ...
    for (const [symbol, poiData] of Object.entries(poiBySymbol)) {
      ...
      if (quarterlyData.length > 0) {
        ...
        try {
          await base44.asServiceRole.entities.PoiDataBinance.create({
            symbol,
            timeframe: '1q',
    ...
    console.log(`Created ${quarterlyRecordsCreated} quarterly records for PoiDataBinance`);

    return Response.json({
      success: true,
      binance: { recordsDeleted, quarterlyRecordsCreated },
      message: `Successfully processed POI data for Binance`
    });
  } catch (error) {
    console.error('updatePoiBinance error:', error);
